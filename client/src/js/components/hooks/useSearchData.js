...
const useSearchData = (defaultSearchOptions, reduxSearchAction) => {
  ...
  const refresh = (reset) => {
    if (reset === true) {
      ...
      if (history) {
        history.push(`?${updateQueryParamsFromHistory(history, _.omit(defaultSearchOptions, ['dataPath']), true)}`);
      }
    } else {
  ...
  useEffect(() => {
    const updateHistoryLocationSearch = () => {
      ...
      history.push(`?${updateQueryParamsFromHistory(history, _.omit(searchOptions, ['dataPath']))}`);
    };

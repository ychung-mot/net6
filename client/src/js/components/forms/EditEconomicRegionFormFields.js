...
const EditEconomicRegionFormFields = ({
  setInitialValues,
  formValues,
  setValidationSchema,
  projectId,
  ...
  return (
    <React.Fragment>
      <FormRow name="ratioRecordLkupId" label={`${ratioTypeName}*`}>
        <SingleDropdownField items={economicRegions} name="ratioRecordLkupId" searchable={true} />
      </FormRow>
...
const mapStateToProps = (state) => {
  return {
    economicRegions: Object.values(state.codeLookups.economicRegions),
    ratioRecordTypes: Object.values(state.codeLookups.ratioRecordTypes),
  };
};

export default connect(mapStateToProps, null)(EditEconomicRegionFormFields);

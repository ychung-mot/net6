...
DROP VIEW IF EXISTS [dbo].[CRT_PROGRAM_PROJECT_FINANCIALS_VW]
GO

DROP VIEW IF EXISTS [dbo].[CRT_PRJ_QTY_ACCOMPLISHMENTS_VW]
GO

DROP VIEW IF EXISTS [dbo].[CRT_PROJECT_TENDERS_VW]
GO

DROP VIEW IF EXISTS [dbo].[CRT_PROJECT_SEGMENTS_VW]
GO

DROP VIEW IF EXISTS [dbo].[CRT_PROJECT_NOTES_VW]
GO

DROP VIEW IF EXISTS [dbo].[CRT_PROJECT_VW]
GO
...
CREATE OR ALTER VIEW [dbo].[CRT_PROGRAM_PROJECT_FINANCIAL_REPORT_VW] AS
...
CREATE OR ALTER VIEW [dbo].[CRT_PRJ_QTY_ACCOMPLISHMENT_REPORT_VW] AS
...
CREATE OR ALTER VIEW [dbo].[CRT_PROJECT_TENDER_REPORT_VW] AS
...
CREATE OR ALTER VIEW [dbo].[CRT_PROJECT_SEGMENT_REPORT_VW] AS
...
CREATE OR ALTER VIEW [dbo].[CRT_PROJECT_NOTE_REPORT_VW] AS
...
CREATE OR ALTER VIEW [dbo].[CRT_PROJECT_RATIO_REPORT_VW] AS      

SELECT
	  prj.PROJECT_ID
	  ,rat.[RATIO_ID]
	  ,rectyp_code.CODE_NAME											AS RATIO_RECORD_TYPE
	  ,rec_code.CODE_NAME												AS RATIO_RECORD_DESC
	  ,sa.SERVICE_AREA_NUMBER
...
FROM
	CRT_PROJECT					prj
	LEFT OUTER JOIN CRT_RATIO rat ON rat.PROJECT_ID = prj.PROJECT_ID
	LEFT OUTER JOIN CRT_CODE_LOOKUP rec_code ON rec_code.CODE_LOOKUP_ID = rat.RATIO_RECORD_LKUP_ID
	LEFT OUTER JOIN CRT_CODE_LOOKUP rectyp_code ON rectyp_code.CODE_LOOKUP_ID = rat.RATIO_RECORD_TYPE_LKUP_ID
	LEFT OUTER JOIN CRT_SERVICE_AREA sa ON sa.SERVICE_AREA_ID = rat.SERVICE_AREA_ID
	LEFT OUTER JOIN CRT_DISTRICT ds ON ds.DISTRICT_ID = rat.DISTRICT_ID;
GO

CREATE OR ALTER VIEW [dbo].[CRT_PROJECT_REPORT_VW] AS
